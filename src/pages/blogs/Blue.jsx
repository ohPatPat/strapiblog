...
export const BlueImg = (props) => {
    ...
    useEffect(() => {
        const getData = async () => {
            ...
            setApiData(result.data.data[0].attributes.Image.data)
            console.log(result.data.data[0].attributes.Image.data);
        }
        getData()
    }, [setApiData])
...
export const Blue = (props) => {
    ...
    useEffect(() => {
        const getData = async () => {
            ...
            const result = await axios.get(url)
            setApiData(result.data.data[1].attributes.blogs.data)
        }
        getData()
    }, [setApiData])
        ...
        apiData ? apiData.map((item, index) => {
            ...
            return (
                <section  key={index} id="Blogz">
                    <h1 style={{ color: "blue",}}>{item.attributes.Title}</h1>
